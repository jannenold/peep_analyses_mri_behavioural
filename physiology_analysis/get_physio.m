function [desmtx, breaths, beats, names,nscans_ind]= get_physio(dir_data,name, k,nscans)
...
elseif strcmp(mode,'mat')
    [puls, resp, scan,nscans_ind] = readlog_mat(name, k, dir_data, samp_int,nscans);
else
...
function [puls,resp,scan,nscans_ind] = readlog_mat(name, k, dir_data, samp_int,nscans)


physio_file = [dir_data [name 'm.mat']];
a           = load(physio_file);
scan        = a.([name '_Ch7']).times;

ind = [0; find(diff(scan)>mean(diff(scan))); size(scan,1)];

if numel(ind) > 5 % 4 runs
    warning('More than 4 runs!');
    ind = [ind(diff(ind) > 300);size(scan,1)];
elseif numel(ind) <= 4 % less than 4 runs
    warning('Less than 4 runs!');
    keyboard;
end

nscans_ind = ind(k+1)-ind(k); % readout how many pulses

if nscans_ind < 300
    error('Less than 300 Pulses in this run.');
    nscans_ind = ind(k+2)-ind(k+1);
    fprintf('Next run has %02d Pulses',nscans_ind);


    correct = find((diff(ind) <= nscans) & (diff(ind) > 300));
    ind     = ind(correct);
    scan    = scan(ind(k)+1:ind(k)+nscans_ind);
...
%now pulse
puls    = a.([name '_Ch1']).values;
dt      = a.([name '_Ch1']).interval;
pulsID  = a.([name '_Ch1']).title;
if ~strcmp(pulsID,'PULS')
    error('Expect channel 2 to be Puls');
end
index = [round(scan(1)./dt) round(scan(end)./dt)+800]; %5 more samples
puls  = puls(index(1):index(2),:);
%now resp
resp    = a.([name '_Ch2']).values;
dt      = a.([name '_Ch2']).interval;
respID  = a.([name '_Ch2']).title;
if ~strcmp(respID,'Resp')
    error('Expect channel 1 to be Resp');
end
index = [round(scan(1)./dt) round(scan(end)./dt)+800];
