function FFX_peep(allSubs,session,model,modelname)
...
for np = 1:size(splitSubs,2)
    ...
    for g = 1:size(splitSubs{np},2)
        ...
        for ses = 1:length(session)
            ...
            for nr = 1:size(allRuns,2)
                ...
                if use_sNIFs == 1
                    ...
                    epiName = ['ra' subName sprintf('_ses-%02.2d',session(ses)) '_task-' studyName '_' runName '_bold.nii'];
                end
                ...
                regressors.(strnameRun) = fullfile(physio_dir, ['complete_reg_' subName sprintf('_ses_%02.2d',session(ses)) '_' sprintf('run_%02d',allRuns(nr)) '.txt']);
        ...
        if specifyFirstLevel == 1
            ...
            for ses = 1:length(session)
                for nr = 1:size(allRuns,2)
                    ...
                    runName = sprintf('run-%02d',nr);
                    temp = ['^ra' subName sprintf('_ses-%02.2d',session(ses)) '_task-' studyName '_' runName '_bold.nii'];
            ...
            coreg_skull_strip = sprintf('s3rbrain_mask.nii');
        ...
        if estimateContrasts == 1
            ...
            if strcmp(basisF,'FIR') && ~estimate_contrasts_heat_pressure
                ...
                %% FIR for all 12 conditions at low int exercise

            elseif strcmp(basisF, 'FIR') && strcmp(modelname,'complete_onset_stimulus')&& estimate_contrasts_heat_pressure
                ...
                for i_fir = 1:n_ints
                    ...
                    tco = tco + 1;
                    con = [];
                    template.spm.stats.con.consess{tco}.tcon.name   = ['Heat30_' num2str(i_fir) '_nacl_lo'];
                    tpl1        = zeros(1,n_ints*6);
                    tpl2        = zeros(1,n_ints*6);
                    tpl1(i_fir) = 1;
                ...
                for i_fir = 1:n_ints
                    ...
                    tco = tco + 1;
                    con = [];
                    template.spm.stats.con.consess{tco}.tcon.name   = ['Heat50_' num2str(i_fir) '_nacl_lo'];
                    tpl1        = zeros(1,n_ints*6);
                    tpl2        = zeros(1,n_ints*6);
                    tpl1(n_ints + i_fir) = 1;
                ...
                for i_fir = 1:n_ints
                    ...
                    tco = tco + 1;
                    con = [];
                    template.spm.stats.con.consess{tco}.tcon.name   = ['Heat70_' num2str(i_fir) '_nacl_lo'];
                    tpl1        = zeros(1,n_ints*6);
                    tpl2        = zeros(1,n_ints*6);
                    tpl1(n_ints*2 + i_fir) = 1;
                ...
                for i_fir = 1:n_ints
                    ...
                    tco = tco + 1;
                    con = [];
                    template.spm.stats.con.consess{tco}.tcon.name   = ['Pressure30_' num2str(i_fir) '_nacl_lo'];
                    tpl1        = zeros(1,n_ints*6);
                    tpl2        = zeros(1,n_ints*6);
                    tpl1(n_ints*3 + i_fir) = 1;
                ...
                for i_fir = 1:n_ints
                    ...
                    tco = tco + 1;
                    con = [];
                    template.spm.stats.con.consess{tco}.tcon.name   = ['Pressure50_' num2str(i_fir) '_nacl_lo'];
                    tpl1        = zeros(1,n_ints*6);
                    tpl2        = zeros(1,n_ints*6);
                    tpl1(n_ints*4 + i_fir) = 1;
                ...
                for i_fir = 1:n_ints
                    ...
                    tco = tco + 1;
                    con = [];
                    template.spm.stats.con.consess{tco}.tcon.name   = ['Pressure70_' num2str(i_fir) '_nacl_lo'];
                    tpl1        = zeros(1,n_ints*6);
                    tpl2        = zeros(1,n_ints*6);
                    tpl1(n_ints*5 + i_fir) = 1;
            ...
            elseif strcmp(basisF, 'HRF') && strcmp(modelname,'complete_onset_stimulus')&& ~estimate_contrasts_heat_pressure
                if ~estimateContrasts_condition
                    ...
                    % Sanity Cstick: Contrast Vector length
                    if length(tConVec) ~= length(condBlock)*8 + sum(n.n_nuis(1:end))
                        error('Contrast Length is not coherent');
                    end
                    ...
                    % Interaction INTENSITY x MODALITY SAL
                    % ===================
                    ...
                    for r = 1:nSess
                        ...
                        %name--------------------------------------------------
                        tcoName = {'inter_intensity_modality_sal'};
                    ...
                    for r = 1:nSess
                        ...
                        %name--------------------------------------------------
                        tcoName = {'inter_intensity_modality_nlx'};
                    ...
                    % Pressure (Nacl and NLX together)
                    %==================
                    ...
                    % Interaction Exercise x TREATMENT Heat
                    % ===================
                    ...
                    for r = 1:nSess
                        ...
                        %name--------------------------------------------------
                        tcoName = {'inter_exercise_treatment_heat'};
                        ...
                        if treatment_order(splitSubs{np}(g)) == 1
                            ...
                            if r < floor(nSess/2)+1
                                ...
                                tConVec = [tConVec block mvmnt subjectConstant];
                    ...
                    % Interaction Exercise x TREATMENT Pressure
                    % ===================
                    ...
                    for r = 1:nSess
                        ...
                        %name--------------------------------------------------
                        tcoName = {'inter_exercise_treatment_pressure'};
                    ...
                    % Interaction Exercise x TREATMENT Heat 70
                    % ===================
                    ...
                    for r = 1:nSess
                        ...
                        %name--------------------------------------------------
                        tcoName = {'inter_exercise_treatment_heat70'};
            ...
            elseif strcmp(basisF, 'HRF') && strcmp(modelname,'complete_onset_stimulus') && estimate_contrasts_heat_pressure
                ...
                %% Interaction MODALITY x Intensity SAL exLO
                %% ===================
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'inter_modality_intensity_sal_exLo'};
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'heat_sal>pressure_sal_exLo'};
            ...
            elseif strcmp(basisF, 'HRF') && strcmp(modelname,'split_onset_stimulus')&& estimate_contrasts_heat_pressure

                nConds = 12;
                nSess = 8;
                tco = 0;

                %% Heat Ex Lo SAL Early
                %%-----------------
                % Prep values
                tConVec = [];
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'pressure_exLo_sal_early'};
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'pressure_exLo_sal_late'};
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'heat_exLo_sal_early>late'};
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'pressure_exLo_sal_early>late'};
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'early_exLo_sal_heat>pressure'};
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'late_exLo_sal_heat>pressure'};
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'heat70_exLo_sal_early>late'};

                    %convec-------------------------------------------------
                    exercise = [params_ses.P.exercise.condition params_ses.P.exercise.condition];
                    condBlock = [0 0 1 zeros(1,3) 0 0 -1 zeros(1,3) 0];
                    negcondBlock = -condBlock;
                    zeroBlock = [zeros(1,nConds) 0];
                ...
                %% Pressure 70 ex Lo Sal Early > Late
                % Prep values
                tConVec = [];
                subjectConstant = [];%zeros(1,nSess);
                params_ses = load(['/projects/crunchie/nold/PEEP/behavioural/MAIN/' sprintf('sub-%02.2d',splitSubs{np}(g)) filesep 'ses-01' filesep sprintf('parameters_sub0%0.02d_bike.mat',splitSubs{np}(g))],'P');
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'pressure70_exLo_sal_early>late'};

                    %convec-------------------------------------------------
                    exercise = [params_ses.P.exercise.condition params_ses.P.exercise.condition];
                    condBlock = [zeros(1,3) 0 0 1 zeros(1,3) 0 0 -1 0];
                    negcondBlock = -condBlock;
                    zeroBlock = [zeros(1,nConds) 0];
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'early_exLo_sal_heat70>pressure70'};

                    %convec-------------------------------------------------
                    exercise = [params_ses.P.exercise.condition params_ses.P.exercise.condition];
                    condBlock = [0 0 1 0 0 -1 zeros(1,6) 0];
                    negcondBlock = -condBlock;
                    zeroBlock = [zeros(1,nConds) 0];
                ...
                for r = 1:nSess
                    ...
                    %name--------------------------------------------------
                    tcoName = {'late_exLo_sal_heat70>pressure70'};

                    %convec-------------------------------------------------
                    exercise = [params_ses.P.exercise.condition params_ses.P.exercise.condition];
                    condBlock = [zeros(1,6) 0 0 1 0 0 -1 0];
                    negcondBlock = -condBlock;
                    zeroBlock = [zeros(1,nConds) 0];
