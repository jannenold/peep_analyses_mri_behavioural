...
%[34 -4 12]; %ant Insula
%[-10 -22 -3]; % Thalamus Lefz
%[18 -15 -27]; %parahippocampal gyrus
%[39 -6 -4.5]; %Posterior Insula
%[-8 -9 66]; %supp motor
%[38 -15 54]; precent gyrus
%[33 -33 26]; postc gyrus
%[8 26 16]; %acc?
%[-32 15 16];
%[-24 -8 -18]; left amygdala
%[-12 4 26];
%[4.5 -36 -9];%pag
...
if barplot == 1
    ...
    plot([xpos; xpos], [val_bar(1,:) val_bar(2,:)], '.k', 'MarkerSize',ms-3);
    % errorbar([1-dev 1+dev; 2-dev 2+dev; 3-dev 3+dev],val_bar,val_se,'.','Color','k', 'LineWidth',lw);
    
    set(gca,'xtick',[1 2],'xticklabel',{'Heat','Pressure'});
    ax = gca;
    ...
    exportgraphics(gcf,sprintf('4betas_pharma_%d_%d_%d.png',coordinate(1),coordinate(2),coordinate(3)),'Resolution',500)
    saveas(gcf,sprintf('4betas_pharma_%d_%d_%d.svg',coordinate(1),coordinate(2),coordinate(3)))
    tbl = table(betas);
    writetable(tbl,sprintf('4betas_pharma_%d_%d_%d.csv',coordinate(1),coordinate(2),coordinate(3)));
