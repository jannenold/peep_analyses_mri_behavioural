...
for l = 1:numel(coordinate)
    ...
    cons = spm_select('FPList',subdir,'s6w_nlco_dartelcon_0028.nii'); % heat eih sal = 28, heat nlx = 32 pressure eih sal = 30
    %con = cons(image_nr,:);
...
if nacl
    col.n = [2 72 115]/255;
    col.n2 = [59 156 217]/255;
else
    col.n2 = [242 149 68]/255;
    col.n = [245 182 106]/255;
end
...
plot(pwc_vec(gender_vec==-1),betas(gender_vec==-1),'.','MarkerSize',ms,'MarkerFaceColor', col.n2,'MarkerEdgeColor',col.n2);hold on;
plot(pwc_vec(gender_vec==1),betas(gender_vec==1),'.','MarkerSize',ms,'MarkerFaceColor', col.n,'MarkerEdgeColor',col.n);hold on;
h = lsline;
set(h(2),'LineWidth',lw,'color',col.n2);
set(h(1),'LineWidth',lw,'color',col.n);
