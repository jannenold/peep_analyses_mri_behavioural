function plot_betas_intensity_modality
...
base_dir = '/projects/crunchie/nold/PEEP/fMRI/Data/MAIN/derivatives/spm_firstlevel/HRF_complete_onset_stimulus_4mm';
...
% inter int treat heat gender: 42 11 0

% Pressure Intensity cons
% ----------------------------
% image_nr = [];
% 54 4 9

% inter int treat pressure cov gender 
% 24 -9 68
...
for g = 1:size(subs,2)
    ...
    cons = spm_select('FPList',subdir,'^s4w_nlco_dartelcon.*.nii$');
    con = cons(image_nr,:);
...
if barplot == 1
    ...
    val_bar = [mean(betas(:,1)) mean(betas(:,2)) mean(betas(:,3))];
    ...
    %plot([xpos], y, '-k', 'LineWidth',lw);
    %plot([xpos], [val_bar(1) val_bar(2) val_bar(3)], '.k', 'MarkerSize',ms-4);
...
%   saveas(gcf,sprintf('6betas_intensity_pressure_%d_%d_%d.svg',coordinate(1),coordinate(2),coordinate(3)))
   tbl = table(betas);
   writetable(tbl,sprintf('heat_pressure_intensity_sal_%d_%d_%d.csv',coordinate(1),coordinate(2),coordinate(3)));

%     %% Calculate t tests
%    % paired t test test between SAL vs. NLX
%      %VAS 30
%     [h,p1,ci,stats1] = ttest(betas(:,1), betas(:,2));
%     % VAS 50
%     [h,p2,ci,stats2] = ttest(betas(:,3), betas(:,4));
%     % VAS 70
%     [h,p3,ci,stats3] = ttest(betas(:,5), betas(:,6));
% 
