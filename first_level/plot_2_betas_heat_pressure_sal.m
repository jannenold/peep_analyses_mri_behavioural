function plot_2_betas_heat_pressure_sal
...
%base_dir = '/projects/crunchie/nold/PEEP/fMRI/Data/MAIN/derivatives/spm_firstlevel/HRF_split_onset_stimulus_6mm_heat_pressure';
...
% heat and pressure Intensity saline shared peak
% ----------------------------
 image_nr = [1 13];

 % heat ex Lo sal: 1
 % pressure ex lo sal: 13

 %split_onset (late pain heat and pressure)
%image_nr = [2 4];

 %split_onset (early pain heat and pressure)
% image_nr = [1 3];
...
if barplot == 1
    ...
    plot([xpos; xpos;], [val_bar(1,:)], '.k', 'MarkerSize',ms-4);
   ...
   %exportgraphics(gcf,sprintf('3betas_intensity_%d_%d_%d.png',coordinate(1),coordinate(2),coordinate(3)),'Resolution',500)
    tbl = table(betas);
    writetable(tbl,sprintf('2betas_heat_pressure_sal_%d_%d_%d.csv',coordinate(1),coordinate(2),coordinate(3)));
